...
function MenuScreen({
  ...
  return (
    <div className="pb-28">
      ...
      {!query && filters.length === 0 && (
        <FeaturedCarousel items={allItems} currency={restaurant.currency} onOpenItem={onOpenItem} />
      )}
...
// A rotating "featured dish" strip at the top of the menu. Real slide
// animation (translateX + transition, plus touch swipe) rather than a hard
// cut between items. Each slide is a full-bleed photo with a gradient for
// legibility -- this works for any dish photo (a golden biryani, a green
// salad, whatever) instead of forcing every photo onto a colored blob that
// only flatters some of them.
function FeaturedCarousel({ items, currency, onOpenItem }) {
  ...
  useEffect(() => setIndex(0), [picks.length]);
  ...
  function goTo(i) {
    setIndex((i + picks.length) % picks.length);
  }
  ...
  return (
    <div className="px-4 pt-4">
      <div
        className="relative overflow-hidden rounded-[28px] shadow-sm bg-clay-light"
          ...
          {picks.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => onOpenItem(item)}
                className="relative w-full flex-shrink-0 h-52 text-left"
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <p className="text-white/85 text-sm mt-0.5 font-medium">{money(price, currency)}</p>
                </div>
              </button>
            );
          })}
  ...
  return (
    <div className="fixed inset-0 z-40 bg-ink/50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        {/* Full-bleed photo, same visual language as the featured carousel */}
        <div className="relative w-full h-64 rounded-t-[32px] overflow-hidden bg-clay-light">
          {item.image_url ? (
            <img src={item.image_url} alt={item.name} className="absolute inset-0 w-full h-full object-cover" />
          ) : (
            <Monogram name={item.name} size="lg" className="rounded-none text-5xl" />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-ink/85 via-ink/5 to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-5 flex items-end justify-between gap-3">
            <h2 className="text-white font-display text-2xl font-bold leading-tight">{item.name}</h2>
            <span className="bg-white rounded-full p-1 flex-shrink-0 shadow">
              <VegDot veg={item.is_veg} />
            </span>
          </div>
        </div>
                ...
                return (
                  <button
                    ...
                    className={`text-xs font-semibold px-3.5 py-2 rounded-full border transition-colors ${
                      active ? "bg-herb border-herb text-white" : "border-ink/15 text-ink/60"
                    }`}
                  >
                    {s}
                  </button>
